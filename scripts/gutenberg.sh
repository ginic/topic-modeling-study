#!/bin/bash
# Adapted from https://www.exratione.com/2014/11/how-to-politely-download-all-english-language-text-format-files-from-project-gutenberg/
# Usage: gutenberg.sh target_dir
#
# Download the complete archive of text format files from Project Gutenberg for a given language.
#
...
if [ ! -f "${ZIP_LIST}" ] ; then
  ...
  # URLs, one per line.
  grep -oh 'http://[a-zA-Z0-9./-]*\.zip' www.gutenberg.org/robot/harvest* | \
    sort | \
    uniq > "${ZIP_LIST}"

  echo "Number of zipfiles to download:"
  wc -l ${ZIP_LIST}
